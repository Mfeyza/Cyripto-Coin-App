...
/* -------------------------------------------------------------------------- */
  ...
  const tr = document.createElement("tr");
      ...
      tr.innerHTML = `
        ...
        ${change > 0 ? `<span style="color:darkgreen;">▲</span>` : '🔻'} ${change}</td>
        <td><a href="${coinrankingUrl}" target="_blank">
        <i class="fa-solid fa-chart-line fa-2x" style="color: rgba(77, 77, 184, 0.776);"></i></a> </td>
      `;
  ...
  modalContent.innerHTML = `
          ...
          <p style="font-size:25px; color:white"><span style="color:blue; font-size:25px"> Change</span>${change}</p>    
          <a href="${coinrankingUrl}" target="_blank">
          <i class="fa-solid fa-chart-line fa-3x"  style="color: #4d8f5a;"></i></a> 
          </div> 
          <div class="d-flex justify-content-center" >
          <button style="background-color: rgb(207, 193, 68); border:none; border-radius:10px;width:80px ; height:40px ;" type="button " id="saveBtn" aria-label="close">Kaydet</button>
          </div>
          </div>
        `;
  ...
  modalheader.innerHTML = `
        <p class=" text-center" style="color:white"> Lütfen geçerli bir değer giriniz</p>
      `;
  }
  });
  ...
  li.innerHTML=`
     <li class="list-group-item" style="borner:none;border-radius:10px; background-image: linear-gradient( 94.3deg,  rgba(26,33,64,1) 10.9%, rgba(81,84,115,1) 87.1% );">${name}</li>`
